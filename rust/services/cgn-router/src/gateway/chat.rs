...
pub async fn completions(
    ...
    let id = format!("chatcmpl-{}", &uuid::Uuid::new_v4().simple().to_string()[..16]);
    ...
    if stream_mode {
        ...
        tokio::spawn(async move {
            if let Err(e) = stream_run(state_clone, proto_req, tx.clone(), id_for_task, model_for_task, created).await {
...
fn build_proto_request(r: &ChatRequest) -> GenerateRequest {
    let messages: Vec<PMessage> = r.messages.iter().map(|m| PMessage {
        role: m.role.clone(),
        content: m.content.clone(),
        name: m.name.clone().unwrap_or_default(),
    }).collect();

    let stops = r.stop.as_ref().cloned().map(|s| s.into_vec()).unwrap_or_default();
            ...
            Ok(t) => {
                let chunk = ChatChunk {
                    ...
                    choices: vec![ChatChunkChoice {
                        ...
                        delta: ChatDelta {
                            role: None,
                            content: if t.text.is_empty() { None } else { Some(t.text.clone()) },
                        },
                        finish_reason: if t.finish.is_empty() { None } else { Some(t.finish.clone()) },
                    }],
                };
                if tx.send(serde_json::to_string(&chunk).unwrap()).await.is_err() {
                    return Ok(());
                }
            }
            Err(e) => {
    ...
    while let Some(item) = stream.next().await {
        match item {
            Ok(t) => {
                ...
                count += 1;
                if !t.finish.is_empty() { finish = t.finish; }
            }
            Err(e) => {
...
fn join_messages(msgs: &[PMessage]) -> String {
    ...
    for m in msgs {
        out.push('<'); out.push_str(&m.role); out.push_str(">\n");
        out.push_str(&m.content); out.push('\n');
    }
    out
}
