...
non-empty content shipped in **v0.3.0** (use **v0.3.1** or newer for MLX
and the latest fixes), so anything older won't
work for `/v1/chat/completions`:

```bash
sed -i.bak 's|cognitora:latest|cognitora:v0.3.1|g' \
